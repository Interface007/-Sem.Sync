...
namespace Sem.Sync.SyncBase.DetailData
{
    ...
    public class PhoneNumber
    {
        ...
        public string DenormalizedPhoneNumber
        {
            get
            {
                return !string.IsNullOrEmpty(this.Number) ? null : this.ToString();
            }
            ...
            set
            {
                ...
                if ((matches.Count > 2) &&
                    Enum.IsDefined(
                        typeof(CountryCode), int.Parse(matches[0].Captures[0].ToString(), CultureInfo.InvariantCulture)))
                {
                    this.CountryCode =
                        (CountryCode)int.Parse(matches[0].Captures[0].ToString(), CultureInfo.InvariantCulture);
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                result = (result * 397) ^
                         (this.denormalizedPhoneNumber != null ? this.denormalizedPhoneNumber.GetHashCode() : 0);
                return result;
            }
        }
