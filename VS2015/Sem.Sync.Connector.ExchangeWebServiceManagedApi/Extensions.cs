...
namespace Sem.Sync.Connector.ExchangeWebServiceManagedApi
{
    ...
    public static class Extensions
    {
        ...
        /// </param>
        /// <returns>
        /// the <see cref="StdContact"/> that did result from this conversion 
        /// </returns>
        public static StdContact ToStdContact(this Item element)
        {
            ...
            result.InternalSyncData = new SyncData
                {
                   DateOfCreation = element.DateTimeCreated, DateOfLastChange = element.LastModifiedTime 
                };

            result.ExternalIdentifier = new ProfileIdentifierDictionary(
                ProfileIdentifierType.ExchangeWs, contact.Id.UniqueId);
            ...
            result.BusinessAddressPrimary =
                GetAddress(contact, PhysicalAddressKey.Business).ToAddressDetail(
                    GetPhone(contact, PhoneNumberKey.BusinessPhone));
            result.PersonalAddressPrimary =
                GetAddress(contact, PhysicalAddressKey.Home).ToAddressDetail(
                    GetPhone(contact, PhoneNumberKey.HomePhone));
            result.PersonalAddressSecondary =
                GetAddress(contact, PhysicalAddressKey.Other).ToAddressDetail(
                    GetPhone(contact, PhoneNumberKey.HomePhone2));
        ...
        /// The exchange contact to be updated. 
        /// </param>
        /// <param name="stdContact">
        /// The <see cref="StdContact"/> to get the information from. 
        /// </param>
        /// <returns>
        /// The updated contact. 
        /// </returns>
        public static Contact UpdateFromStdContact(this Contact exchangeContact, StdContact stdContact)
        {
            ...
            exchangeContact.SetPhoneNumber(
                PhoneNumberKey.HomePhone, stdContact.PersonalAddressPrimary.NewIfNull().Phone.NewIfNull().ToString());
            exchangeContact.SetPhoneNumber(
                PhoneNumberKey.HomePhone2, stdContact.PersonalAddressSecondary.NewIfNull().Phone.NewIfNull().ToString());
            exchangeContact.SetPhoneNumber(
                PhoneNumberKey.BusinessPhone, stdContact.BusinessAddressPrimary.NewIfNull().Phone.NewIfNull().ToString());
            exchangeContact.SetPhoneNumber(
                PhoneNumberKey.BusinessPhone2, 
                stdContact.BusinessAddressSecondary.NewIfNull().Phone.NewIfNull().ToString());

            exchangeContact.SetPhoneNumber(
                PhoneNumberKey.MobilePhone, stdContact.BusinessPhoneMobile.NewIfNull().ToString());
            exchangeContact.SetPhoneNumber(
                PhoneNumberKey.OtherTelephone, stdContact.PersonalPhoneMobile.NewIfNull().ToString());

            exchangeContact.SetImAddress(
                ImAddressKey.ImAddress1, stdContact.BusinessInstantMessengerAddresses.NewIfNull().MsnMessenger);
            exchangeContact.SetImAddress(
                ImAddressKey.ImAddress2, stdContact.PersonalInstantMessengerAddresses.NewIfNull().MsnMessenger);
            exchangeContact.SetImAddress(
                ImAddressKey.ImAddress3, stdContact.PersonalInstantMessengerAddresses.NewIfNull().GoogleTalk);
