...
namespace Sem.Sync.Connector.Filesystem
{
    ...
    /// </summary>
    /// <typeparam name="T">
    /// A class that does inherit from StdElement. Elements of this type can be 
    ///   loaded/saved with an instance of this class
    /// </typeparam>
    [ClientStoragePathDescription(Mandatory = true, Default = "{FS:WorkingFolder}\\Elements.xml", 
        ReferenceType = ClientPathType.FileSystemFileNameAndPath)]
    [ConnectorDescription(DisplayName = "Filesystem generic client", IsGeneric = true, CanReadCalendarEntries = true, 
        CanWriteCalendarEntries = true)]
    public class GenericClient<T> : StdClient, IBackupStorage
        where T : StdElement
    {
        ...
        {
            var destFileName = clientFolderName +
                               string.Format(
                                   CultureInfo.InvariantCulture, 
                                   "-{0:yyyy-MM-dd-hh-mm-ss}-{1}.syncbackup", 
                                   DateTime.Now, 
                                   Guid.NewGuid());
