...
namespace Facebook.Parser {
    internal sealed class LocationParser
    {
        /// <summary>
        /// Default constructor
        /// </summary>
        ...
        internal static Location ParseLocation(XmlNode node)
        {
            ...
            if (node != null)
            {
                ...
                if (!String.IsNullOrEmpty(XmlHelper.GetNodeText(node, "country")))
                {
                    try
                    {
                        location.Country = (Country)Enum.Parse(typeof(Country), XmlHelper.GetNodeText(node, "country").Replace(" ", "").Replace(".", "").Replace("'", "").Replace("-", ""), true);
