...
namespace Sem.Sync.SyncBase.Commands
{
    ...
    public class MatchByEMail : SyncComponent, ISyncCommand
    {
        ...
        private static readonly Regex CheckRegEx =
            new Regex(
                @"([^a-zA-Z]?(test)|(promo.?)|(info)|(support)|(redaktion)|(hilfe)[^a-zA-Z])|(abteilung)|(service)@", 
        ...
        /// </param>
        /// <returns>
        /// true if comparison is allowed 
        /// </returns>
        private static bool IsValidEmailAddress(string emailAddress)
        {
            return !string.IsNullOrEmpty(emailAddress) && !CheckRegEx.IsMatch(emailAddress);
        }
        ...
        private List<StdElement> MatchThisByEMail(IEnumerable<StdContact> source, List<StdContact> target)
        {
            foreach (var item in target)
            {
                ...
                corresponding =
                    (from element in source where IsEmailMatch(element, testItem) select element).FirstOrDefault();
