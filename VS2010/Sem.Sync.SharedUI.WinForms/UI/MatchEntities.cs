...
namespace Sem.Sync.SharedUI.WinForms.UI
{
    ...
    public partial class MatchEntities : Form
    {
            ...
            ProfileIdentifierType identifierToUse)
        {
            ...
            return this.ShowDialog() != DialogResult.OK ? null : this.matching.BaseLine.ToStdElements();
        }
        ...
        private void BtnMatch_Click(object sender, EventArgs e)
        {
            try
            {
                this.lastSelectedSourceRow = this.dataGridSourceCandidates.CurrentRow == null
                                                 ? 0
                                                 : this.dataGridSourceCandidates.CurrentRow.Index;

                this.lastSelectedTargetRow = this.dataGridTargetCandidates.CurrentRow == null
                                                 ? 0
        ...
        /// </param>
        /// <returns>
        /// true if there has been selected a target row automatically, too
        /// </returns>
        private bool SelectSourceRow(DataGridViewRow row)
        {
            ...
            var autoMatch = (from x in this.dataGridTargetCandidates.Rows.Cast<DataGridViewRow>()
                             where
                                 ((MatchCandidateView)x.DataBoundItem).Element.ToStringSimple() ==
        ...
        private void SetupGui()
        {
            ...
            if (this.lastSelectedSourceRow > 0 && this.lastSelectedSourceRow < this.dataGridSourceCandidates.RowCount &&
                this.dataGridSourceCandidates.RowCount > 1)
            {
                this.dataGridSourceCandidates.Rows[this.lastSelectedSourceRow].Selected = true;
                this.dataGridSourceCandidates.CurrentCell =
                    this.dataGridSourceCandidates.Rows[this.lastSelectedSourceRow].Cells[0];
            }
            ...
            if (!found && this.lastSelectedTargetRow > 0 &&
                this.lastSelectedTargetRow < this.dataGridTargetCandidates.RowCount)
            {
                this.dataGridTargetCandidates.Rows[this.lastSelectedTargetRow].Selected = true;
                this.dataGridTargetCandidates.CurrentCell =
                    this.dataGridTargetCandidates.Rows[this.lastSelectedTargetRow].Cells[0];
            }
