// --------------------------------------------------------------------------------------------------------------------
// <copyright file="AnalysisStdCalendarItem.cs" company="Sven Erik Matzen">
...
namespace Sem.Sync.Connector.Statistic.AnalysisModule
{
    ...
    public class StdCalendarItems
    {
        ...
        /// </summary>
        /// <param name="calendarItems"> The list of calendar items to be analyzed. </param>
        /// <returns> a new instance of the analysis results entity or null </returns>
        public static StdCalendarItems GetAnalysisItemResult(List<StdCalendarItem> calendarItems)
        {
            ...
            return new StdCalendarItems
                        {
                            NextSevenDays =     (from x in calendarItems where x.Start >= DateTime.Now && x.Start <= DateTime.Now.AddDays(7) select x).Count(),
                            NextFourteenDays =  (from x in calendarItems where x.Start >= DateTime.Now && x.Start <= DateTime.Now.AddDays(14) select x).Count()
                        };
        }
    }
}
